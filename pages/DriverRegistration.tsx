...
import { Truck, ArrowRight, CheckCircle, Upload, Lock, Loader2 } from 'lucide-react';
...
const DriverRegistration: React.FC<DriverRegistrationProps> = ({ onComplete, onCancel }) => {
  ...
  const handleSubmit = async () => {
    ...
    const newUser = {
      ...
      role: 'driver',
      // Metadata adicional que el backend pueda manejar
      metadata: {
        cdl: formData.cdlNumber,
        state: formData.state
      }
    };

    const result = await DatabaseService.registerUser(newUser);
    setLoading(false);
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
            <div className="grid grid-cols-2 gap-4">
               <div className="col-span-2">
                 ...
                 <input 
                    className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:border-orange-500 outline-none" 
                    value={formData.name}
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
            ...
            <div className="pt-4 border-t border-slate-800">
               ...
               <div className="relative mt-1">
                 ...
                 <input 
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 pl-10 text-white focus:border-orange-500 outline-none" 
                    type="password" 
            ...
            <div className="flex gap-3">
               ...
               <button 
                  ...
                  disabled={loading}
                  className="flex-1 bg-orange-500 hover:bg-orange-400 text-slate-950 font-bold py-3 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50"
               >
                 {loading ? <Loader2 className="animate-spin" /> : <><CheckCircle size={20} /> Register Driver</>}
               </button>
            </div>
          </div>
        )}
